    ...
    <form hx-post="/reading-logs" hx-target="#modal-target" hx-swap="outerHTML">
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <input type="datetime-local" id="rl-read-at" name="read_at" value="` + time.Now().Format("2006-01-02T15:04") + `" class="w-full px-3 py-2 rounded-lg border bg-surface text-sm" style="border-color: var(--color-secondary);">
        </div>
        <div>
...
func HTMLCreateReadingLogHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if readAt != "" {
			// Convert datetime-local format to our timestamp format
			t, err := time.Parse("2006-01-02T15:04", readAt)
			if err != nil {
				// Try other formats
				t, err = time.Parse("2006-01-02 15:04:05", readAt)
				if err != nil {
					readAt = time.Now().Format("2006-01-02 15:04:05")
				} else {
					readAt = t.Format("2006-01-02 15:04:05")
				}
			} else {
				readAt = t.Format("2006-01-02 15:04:05")
			}
